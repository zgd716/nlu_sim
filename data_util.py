...
def token_string_as_list(string,tokenize_style='char',stopword_file='data/stopwords.txt'):
    #停用词  jellyzhang
    stopwords=[]
    with open(stopword_file,'r')as fread:
        for line in fread:
    ...
    if tokenize_style=='char':
        ...
        listt=[word for word in jieba.lcut(string) if word not in stopwords] # jellyzhang  过滤停用词
...
def data_mining_features(index,input_string_x1,input_string_x2,vocab_word2index,word_vec_fasttext_dict,word_vec_word2vec_dict,tfidf_dict,n_gram=8):
    ...
    #7 jellyzhang  2018.05.16  添加jaccard
    # jaccard=cal_j(x1_list,x2_list)
    # feature_list.extend([jaccard])
